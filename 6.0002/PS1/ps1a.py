...
def brute_force_cow_transport(cows,limit=10):
    ...
    for partition in get_partitions(cows_by_names):
        weight_over_limt = False
        for sublist in partition:
            weight = 0 
            for cow in sublist:
                weight += copy_cows[cow]
            if weight > limit:
                weight_over_limt is True
                break
        if weight_over_limt is True:
            continue
...
def compare_cow_transport_algorithms():
    ...
    print(f'Exectution time for greedy algorithm was {end - start}')
    print(f'Problem was solved within {len(result)} trips.')
    start = time.time()
    result = brute_force_cow_transport(cows)
    end = time.time()
    print(f'Exectution time for brute force algorithm was {end - start}')
    print(f'Problem was solved within {len(result)} trips.')
